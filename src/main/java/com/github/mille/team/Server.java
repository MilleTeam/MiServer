...
import com.github.mille.team.block.entity.*;
import com.github.mille.team.command.*;
import com.github.mille.team.configuration.Configuration;
import com.github.mille.team.configuration.ConfigurationSection;
import com.github.mille.team.console.NukkitConsole;
...
import com.github.mille.team.entity.mob.*;
import com.github.mille.team.entity.passive.*;
import com.github.mille.team.entity.projectile.EntityArrow;
import com.github.mille.team.entity.projectile.EntityEnderPearl;
import com.github.mille.team.entity.projectile.EntitySnowball;
import com.github.mille.team.event.HandlerList;
import com.github.mille.team.event.level.LevelInitEvent;
import com.github.mille.team.event.level.LevelLoadEvent;
import com.github.mille.team.event.server.QueryRegenerateEvent;
import com.github.mille.team.inventory.CraftingManager;
import com.github.mille.team.inventory.Recipe;
import com.github.mille.team.item.Item;
import com.github.mille.team.item.enchantment.Enchantment;
import com.github.mille.team.lang.BaseLang;
...
import com.github.mille.team.level.format.LevelProvider;
import com.github.mille.team.level.format.LevelProviderManager;
import com.github.mille.team.level.format.anvil.Anvil;
import com.github.mille.team.level.format.leveldb.LevelDB;
import com.github.mille.team.level.format.mcregion.McRegion;
import com.github.mille.team.level.generator.Flat;
import com.github.mille.team.level.generator.Generator;
import com.github.mille.team.level.generator.Nether;
import com.github.mille.team.level.generator.Normal;
import com.github.mille.team.level.generator.biome.Biome;
import com.github.mille.team.math.NukkitMath;
import com.github.mille.team.metadata.EntityMetadataStore;
import com.github.mille.team.metadata.LevelMetadataStore;
import com.github.mille.team.metadata.PlayerMetadataStore;
...
import com.github.mille.team.network.CompressBatchedTask;
import com.github.mille.team.network.Network;
import com.github.mille.team.network.RakNetInterface;
import com.github.mille.team.network.SourceInterface;
import com.github.mille.team.network.protocol.BatchPacket;
import com.github.mille.team.network.protocol.DataPacket;
import com.github.mille.team.network.protocol.PlayerListPacket;
import com.github.mille.team.network.protocol.ProtocolInfo;
import com.github.mille.team.network.query.QueryHandler;
import com.github.mille.team.network.rcon.RCON;
import com.github.mille.team.permission.BanEntry;
import com.github.mille.team.permission.BanList;
import com.github.mille.team.permission.DefaultPermissions;
import com.github.mille.team.permission.Permissible;
import com.github.mille.team.plugin.JavaPluginLoader;
import com.github.mille.team.plugin.Plugin;
import com.github.mille.team.plugin.PluginLoadOrder;
import com.github.mille.team.plugin.PluginManager;
import com.github.mille.team.plugin.service.NKServiceManager;
import com.github.mille.team.plugin.service.ServiceManager;
import com.github.mille.team.potion.Effect;
import com.github.mille.team.potion.Potion;
import com.github.mille.team.resourcepacks.ResourcePackManager;
import com.github.mille.team.scheduler.FileWriteTask;
import com.github.mille.team.scheduler.ServerScheduler;
import com.github.mille.team.scheduler.Task;
import com.github.mille.team.utils.*;
import com.github.mille.team.utils.bugreport.ExceptionHandler;
...
public class Server {
    ...
    public boolean allowInventoryCheats = false;

    private BanList banByName = null;

    private BanList banByIP = null;

    private Configuration operators = null;

    private Configuration whitelist = null;
    ...
    Server(final String filePath, String dataPath, String pluginPath, String predefinedLanguage) {
        ...
        if (!new File(this.dataPath + "mi-server.yml").exists()) {
            this.getLogger().info("Welcome! Please choose a language first!");
            try {
            ...
            InputStream advancedConf = this.getClass().getClassLoader().getResourceAsStream("lang/" + language + "/mi-server.yml");
            if (advancedConf == null) {
                advancedConf = this.getClass().getClassLoader().getResourceAsStream("lang/" + fallback + "/mi-server.yml");
            }

            try {
                Utils.writeFile(this.dataPath + "mi-server.yml", advancedConf);
            } catch (IOException e) {
                throw new RuntimeException(e);
            }
        }
        ...
        log.info("Loading MiServer configuration...");
        this.config = new Configuration(this.dataPath + "mi-server.yml", Configuration.YAML);
        ...
        this.properties = new Configuration(this.dataPath + "server.properties", Configuration.PROPERTIES, new ConfigurationSection() {

            {
                put("motd", "A server software for Minecraft Pocket Edition");
        ...
        log.info(getLanguage().translateString("nukkit.server.start", this.getVersion()));
        ...
        this.operators = new Configuration(this.dataPath + "ops.txt", Configuration.ENUM);
        this.whitelist = new Configuration(this.dataPath + "white-list.txt", Configuration.ENUM);
        ...
        log.info(this.getLanguage().translateString("nukkit.server.info", this.getName(), this.getNukkitVersion(), this.getCodename(), this.getApiVersion()));
    ...
    public void updatePlayerListData(UUID uuid, long entityId, String name, Skin skin, Collection<Player> players) {
        this.updatePlayerListData(uuid, entityId, name, skin, players.stream().filter(p -> !p.getUniqueId().equals(uuid)).toArray(Player[]::new));
    }
    ...
    public void sendFullPlayerListData(Player player) {
        ...
        PlayerListPacket pk = new PlayerListPacket();
        pk.type = PlayerListPacket.TYPE_ADD;
        pk.entries = this.playerList.values().stream().filter(p -> !p.getUniqueId().equals(uuid)).map(p -> new PlayerListPacket.Entry(p.getUniqueId(), p.getId(), p.getDisplayName(), p.getSkin())).toArray(PlayerListPacket.Entry[]::new);
    ...
    public void titleTick() {
        ...
        String title = (char) 0x1b + "]0;" + this.getName() + " " + this.getNukkitVersion() + " | Online " + this.players.size() + "/" + this.getMaxPlayers() + " | Memory " + usage;
        if (!Nukkit.shortTitle) {
            title += " | U " + NukkitMath.round((this.network.getUpload() / 1024 * 1000), 2) + " D " + NukkitMath.round((this.network.getDownload() / 1024 * 1000), 2) + " kB/s";
        }
    ...
    public String getMotd() {
        return this.getPropertyString("motd", "A server software for Minecraft Pocket Edition");
    }
    ...
    public boolean generateLevel(String name, long seed, Class<? extends Generator> generator, Map<String, Object> options, Class<? extends LevelProvider> provider) {
        ...
        if (provider == null) {
            if ((provider = LevelProviderManager.getProviderByName(this.getConfig("level-settings.default-format", "anvil"))) == null) {
