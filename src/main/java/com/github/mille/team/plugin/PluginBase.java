...
import com.github.mille.team.command.PluginIdentifiableCommand;
...
/**
 * 一般的Nukkit插件需要继承的类。<br> A class to be extended by a normal Nukkit plugin.
 *
...
abstract public class PluginBase implements Plugin {
    ...
    /**
     * 加载或卸载这个插件。<br> Enables or disables this plugin.
     * <p>
    ...
    /**
     * 初始化这个插件。<br> Initialize the plugin.
     * <p>
     ...
     *
     * @param loader      加载这个插件的插件加载器的{@code PluginLoader}对象。<br> The plugin loader ,which loads this plugin, as a {@code PluginLoader} object.
     * @param server      运行这个插件的服务器的{@code Server}对象。<br> The server running this plugin, as a {@code Server} object.
     * @param description 描述这个插件的{@code PluginDescription}对象。<br> A {@code PluginDescription} object that describes this plugin.
     * @param dataFolder  这个插件的数据的文件夹。<br> The data folder of this plugin.
     * @param file        这个插件的文件{@code File}对象。对于jar格式的插件，就是jar文件本身。<br> The {@code File} object of this plugin itself. For jar-packed plugins, it is the jar file itself.
     * @since Nukkit 1.0 | Nukkit API 1.0.0
     */
    ...
    /**
     * 返回这个插件是否已经初始化。<br> Returns if this plugin is initialized.
     *
     ...
     * @see com.github.mille.team.plugin.PluginDescription#getFullName
     * @since Nukkit 1.0 | Nukkit API 1.0.0
     */
    ...
    /**
     * 返回这个插件的文件{@code File}对象。对于jar格式的插件，就是jar文件本身。<br> Returns the {@code File} object of this plugin itself. For jar-packed plugins, it is the jar file itself.
     *
