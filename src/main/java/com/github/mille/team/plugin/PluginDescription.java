...
import com.github.mille.team.utils.PluginException;
...
/**
 * 描述一个Nukkit插件的类。<br> Describes a Nukkit plugin.
 * <p>
 ...
 * - TestPlugin1
 * </pre></blockquote>
 * 在使用plugin.yml来定义插件时，{@code name}、{@code main}、{@code version}、{@code api}这几个字段是必需的， 要让Nukkit能够正常加载你的插件，必须要合理地填写这几个字段。<br> When using plugin.yml file to define your plugin, it's REQUIRED to fill these items: {@code name},{@code main},{@code version} and {@code api}.You are supposed to fill these items to make sure your plugin can be normally loaded by Nukkit.<br>
 * </p>
 * <p>
...
public class PluginDescription {
    ...
    /**
     * 返回这个插件完整的名字。<br> Returns the full name of this plugin.
     * <p>
    ...
    /**
     * 返回这个插件支持的Nukkit API版本列表。<br> Returns all Nukkit API versions this plugin supports.
     *
    ...
    /**
     * 返回这个插件的作者列表。<br> Returns all the authors of this plugin.
     *
    ...
    /**
     * 返回这个插件的信息前缀。<br> Returns the message title of this plugin.
     * <p>
     * <p>插件的信息前缀在记录器记录信息时，会作为信息头衔使用。如果没有定义记录器，会使用插件的名字作为信息头衔。<br>
     * When a PluginLogger logs, the message title is used as the prefix of message. If prefix is undefined, the plugin name will be used instead. </p>
     *
     * @return 这个插件的作信息前缀。如果没定义，返回{@code null}。<br> The message title of this plugin, or{@code null} if undefined.
     * @see PluginLogger
     * @see PluginDescription
     * @since Nukkit 1.0 | Nukkit API 1.0.0
     */
    ...
    /**
     * 返回这个插件定义的命令列表。<br> Returns all the defined commands of this plugin.
     *
    ...
    /**
     * 返回这个插件所依赖的插件名字。<br> The names of the plugins what is depended by this plugin.
     * <p>
    ...
    /**
     * 返回这个插件的描述文字。<br> Returns the description text of this plugin.
     *
    ...
    /**
     * 返回这个插件的主类名。<br> Returns the main class name of this plugin.
     * <p>
     * <p>一个插件的加载都是从主类开始的。主类的名字在插件的配置文件中定义后可以通过这个函数返回。一个返回值例子：<br>
     * The load action of a Nukkit plugin begins from main class. The name of main class should be defined in the plugin configuration, and it can be returned by this function. An example for return value: <br> {@code "com.example.ExamplePlugin"}</p>
     *
    ...
    /**
     * 返回这个插件加载的顺序，即插件应该在什么时候加载。<br> Returns the order the plugin loads, or when the plugin is loaded.
     *
    ...
    /**
     * 返回这个插件定义的权限列表。<br> Returns all the defined permissions of this plugin.
     *
    ...
    /**
     * 返回这个插件的版本号。<br> Returns the version string of this plugin.
     *
    ...
    /**
     * 返回这个插件的网站。<br> Returns the website of this plugin.
     *
