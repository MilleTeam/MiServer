package com.github.mille.team.scheduler;
 ...
 * A task can be executed by Nukkit server with a/an express, delay, repeat or delay&repeat. See:{@link ServerScheduler}</p>
 * <p>
 * <p>对于插件开发者，为确保自己任务能够在安全的情况下执行（比如：在插件被禁用时不执行），
 * 建议让任务继承{@link PluginTask}类而不是这个类。<br> For plugin developers: To make sure your task will only be executed in the case of safety (such as: prevent this task from running if its owner plugin is disabled), it's suggested to use {@link PluginTask} instead of extend this class.</p>
 *
