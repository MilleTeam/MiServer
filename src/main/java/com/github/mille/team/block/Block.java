 ...
 */
public abstract class Block extends Position implements Metadatable, Cloneable, BlockIds {
    ...
    private static boolean correctTool0(
        int blockToolType,
        Item item
    ) {
    ...
    private static double breakTime0(
        ...
        int toolType,
        int toolTier,
        int efficiencyLoreLevel,
        int hasteEffectLevel,
        boolean insideOfWaterWithoutAquaAffinity,
        boolean outOfWaterButNotOnGround
    ) {
    ...
    public boolean isTopFacingSurfaceSolid() {
        if (this.isSolid()) {
            return true;
        } else {
            if ((this instanceof BlockStairs) && (this.getDamage() & 4) == 4) {
                return true;
            } else if ((this instanceof BlockSlab) && (this.getDamage() & 8) == 8) {
                return true;
            } else return (this instanceof BlockSnowLayer) && (this.getDamage() & 7) == 7;
        }
    }
    ...
    public boolean canNeighborBurn() {
        int face = 0;
        while (face < 5) {
            if (this.getSide(BlockFace.fromIndex(face)).getBurnChance() > 0) {
                return true;
            }
            face++;
        }
    ...
    public void addVelocityToEntity(
        Entity entity,
        Vector3 vector
    ) {
    ...
    public MovingObjectPosition calculateIntercept(
        Vector3 pos1,
        Vector3 pos2
    ) {
