...
/**
 * 所有的监听事件的类必须实现的接口。<br> An interface implemented by all classes that handles events.
 * <p>
 * <p>插件要监听事件，需要一个类实现这个接口，在这个类里编写方法来监听。这个类称作<b>监听类</b>。
 * 监听类中监听事件的方法称作事件的<b>处理器</b>。一个监听类可以包含多个不同的事件处理器。 实现监听类后，插件需要在插件管理器中注册这个监听类。<br> If a plugin need to listen events, there must be a class implement this interface. This class is called a <b>listener class</b>. Methods with specified parameters should be written in order to listen events. This method is called a <b>handler</b>. One listener class could contain many different handlers. After implemented the listener class, plugin should register it in plugin manager.</p>
 * <p>
 ...
 * Here is an example for writing a listener class and a handler method. Note that for the handler, tag {@code @EventHandler} and the parameter is required:</p>
 * <pre>
 ...
 * </pre></p>
 *
 * <p>关于注册监听类，请看：{@link com.github.mille.team.plugin.PluginManager#registerEvents}.<br>
 * For registering listener class, See: {@link com.github.mille.team.plugin.PluginManager#registerEvents}.</p>
 *
