...
import com.github.mille.team.level.generator.biome.CaveBiome;
...
//								if (this.isSuitableBlock(material, materialAbove, biome))
    ...
    protected boolean isSuitableBlock(
        int block,
        int blockAbove,
        Biome biome
    ) {
        if (!(biome instanceof CaveBiome caveBiome)) {
            return false;
        }
    ...
    protected void generateChunk(
        int chunkX,
        int chunkZ,
        FullChunk generatingChunkBuffer
    ) {
