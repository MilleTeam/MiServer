...
public class ConfigurationSection extends LinkedHashMap<String, Object> {
     ...
     * @param map
     */
    public ConfigurationSection(LinkedHashMap<String, Object> map) {
        this();
        if (map == null || map.isEmpty()) return;
        for (Map.Entry<String, Object> entry : map.entrySet()) {
            if (entry.getValue() instanceof LinkedHashMap) {
                super.put(entry.getKey(), new ConfigurationSection((LinkedHashMap) entry.getValue()));
            } else {
    ...
    public <T> T get(
        ...
        if (value != null && value instanceof ConfigurationSection section) {
    ...
    public void set(
        ...
        if (subKeys.length > 1) {
            ...
            if (this.containsKey(subKeys[0]) && super.get(subKeys[0]) instanceof ConfigurationSection)
    ...
    public ConfigurationSection getSection(String key) {
        return this.get(key, new ConfigurationSection());
    }
    ...
    public ConfigurationSection getSections(String key) {
        ...
        ConfigurationSection parent = key == null || key.isEmpty() ? this.getAll() : getSection(key);
    ...
    public void remove(String key) {
        ...
        if (super.containsKey(key)) super.remove(key);
            ...
            String[] keys = key.split("\\.", 2);
            if (super.get(keys[0]) instanceof ConfigurationSection section) {
    ...
    public Set<String> getKeys(boolean child) {
        ...
        this.entrySet().forEach(entry -> {
            ...
            if (entry.getValue() instanceof ConfigurationSection) {
                if (child)
                    ((ConfigurationSection) entry.getValue()).getKeys(true).forEach(childKey -> keys.add(entry.getKey() + "." + childKey));
            }
        });
        return keys;
    }
