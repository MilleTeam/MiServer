...
import com.github.mille.team.permission.ServerOperator;

/**
 * 用来描述一个玩家和获得这个玩家相应信息的接口。<br> An interface to describe a player and get its information.
 * <p>
...
public interface IPlayer extends ServerOperator, Metadatable {
     ...
     * Notice that this will only return its login name. If you need its display name, turn to {@link Player#getDisplayName}</p>
     *
     ...
     *
     * @param value 如果为{@code true}，封禁这个玩家。如果为{@code false}，解封这个玩家。<br> {@code true} for ban and {@code false} for pardon.
     * @see #isBanned
     * @since Nukkit 1.0 | Nukkit API 1.0.0
     */
    ...
    /**
     * 把这个玩家加入白名单，或者取消这个玩家的白名单。<br> Adds this player to the white list, or removes it from the whitelist.
     *
     * @param value 如果为{@code true}，把玩家加入白名单。如果为{@code false}，取消这个玩家的白名单。<br> {@code true} for add and {@code false} for remove.
     * @see #isWhitelisted
